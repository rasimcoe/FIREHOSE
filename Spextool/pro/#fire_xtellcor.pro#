...
case uvalue of
    ...
    'Shift All': fire_xtellcor_allshift,state
...
if (1) then begin ; beta fix of scalelines problem...this should be turned on eventually.

stdidx=stdidx[sort(stdidx)]
objidx=objidx[sort(stdidx)]

; Order numbers are counting down from 31, 
; but kernels are ascending in wavelength, so there is a mismatch
; Fix by reordering the kernels
kernels=*state.d.kernels
for i = 0, n_elements(stdidx)-1 DO BEGIN
  tag = (tag_names(kernels))[stdidx[i]]
  tmpkern = kernels.(stdidx[i])
  newkern = (i eq 0) ? create_struct(tag,tmpkern):create_struct(newkern,tag,tmpkern)
endfor

; Now call xscalelines with the reordered kernel
xscalelines,(*state.d.stdspec)[*,*,stdidx],(*state.d.stdorders)[stdidx],$
            state.r.vmag,(state.r.bmag-state.r.vmag),wvega,fvega,fcvega, $
            *state.d.cf2vega,newkern,vshift,(*state.d.objspec)[*,*,objidx], $
...
; Rewrite the kernel to reflect the reordering.
*state.d.kernels=newkern
...
;******************************************************************************
;
pro fire_xtellcor_allshift,state
...
minwv = [0,0,0,0,0.945,0,0,1.095,1.110,1.160,0,1.31,0,1.42,0,0,1.75,0,1.95,0,0]
maxwv = [0,0,0,0,0.970,0,0,1.110,1.160,1.200,0,1.33,0,1.49,0,0,1.80,0,2.05,0,0]
ctr = 0

for iorder=31L, 11L, -1L do begin

   ; Wavelength ranges to use for correlation
   ; 31: skip
   ; 30: skip
   ; 29: skip
   ; 28: skip
   ...
   z = where((*state.d.objorders) eq iorder)
   ...
   if (minwv[ctr] GT 0) then begin
      shift = fire_xfindshift_noninteractive(obj_wave,obj_flux,new_flux,$
                             INITSHIFT=total((*state.r.shift)[z,state.r.plotobjap]),$
                             XTITLE=state.r.xtitle,PARENT=state.w.fire_xtellcor_base, $
                             CANCEL=cancel,minwv=minwv[ctr],maxwv=maxwv[ctr])
      print, "Shifting order ", iorder, " by ", shift
      (*state.r.shift)[z,state.r.plotobjap] = shift
   endif else begin
      print, "No shift for order ", iorder
   endelse
