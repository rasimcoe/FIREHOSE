...
;     8/25/11 - modified by A. Burgasser to work with FIRE LD
;-
...
;******************************************************************************
;
pro fire_xtellcor_ld_event, event
...
;    'Construct Kernel': fire_xtellcor_ld_conkernel,state
    
    'Construct Telluric Spectra': fire_xtellcor_ld_contellspec,state

    'Get Shift': fire_xtellcor_ld_getshift,state

    'Help': fire_xtellcor_ld_help,state

    'Load Spectra': fire_xtellcor_ld_loadspec,state
    ...
    'Object Spectra Button': begin
        ...
        obj = dialog_pickfile(DIALOG_PARENT=state.w.fire_xtellcor_ld_base,$
    ...
    'Path Button': begin
        ...
        path= dialog_pickfile(/DIRECTORY,$
                              DIALOG_PARENT=state.w.fire_xtellcor_ld_base,$
                              TITLE='Select Path',/MUST_EXIST)
        
        if path ne '' then begin

            path = cpath(path,WIDGET_ID=state.w.fire_xtellcor_ld_base,$
                         CANCEL=cancel)
            if cancel then return
    ...
    'Scale Lines': fire_xtellcor_ld_getscales,state
    ...
    'Standard Spectra Button': begin
        ...
        path = cfld(state.w.path_fld,7,CANCEL=cancel)
        if cancel then return
        std = dialog_pickfile(DIALOG_PARENT=state.w.fire_xtellcor_ld_base,$
                             PATH=path,/MUST_EXIST,FILTER='*,fits')
...
;
;    end
    'Write File': fire_xtellcor_ld_writefile,state
...
cont: 
widget_control, state.w.fire_xtellcor_ld_base, SET_UVALUE=state, /NO_COPY
getout:
...
;******************************************************************************
;
pro fire_xtellcor_ld_cleanup,base
...
;******************************************************************************
;
pro fire_xtellcor_ld_changeunits,wave,tellspec,tellspec_error,state
...
;******************************************************************************
;
pro fire_xtellcor_ld_conkernel,state
...
;******************************************************************************
;
pro fire_xtellcor_ld_contellspec,state
...
fire_xtellcor_ld_changeunits,(*state.d.stdspec)[*,0],tellcor,tellcor_error,$
  state
...
;******************************************************************************
;
pro fire_xtellcor_ld_getscales,state
...
xscalelines,*state.d.stdspec,[1],state.r.vmag,$
  ...
  *state.d.kernels,vshift,*state.d.objspec,[1],1,state.d.awave,$
  state.d.atrans,state.r.hlines,state.r.hnames,state.r.scale,$
            ...
            PARENT=state.w.fire_xtellcor_ld_base,$
            CANCEL=cancel
...
;******************************************************************************
;
pro fire_xtellcor_ld_help,state

    
openr, lun, filepath('fire_xtellcor_ld_helpfile.txt',$
                     ROOT_DIR=state.r.packagepath,SUBDIR='helpfiles'),$
       /get_lun
nlines = numlines(filepath('fire_xtellcor_ld_helpfile.txt',$
...
xmc_displaytext,array,TITLE='Xvspec Help File', $
                GROUP_LEADER=state.w.fire_xtellcor_ld_base

end
;
;******************************************************************************
;
pro fire_xtellcor_ld_loadspec,state
...
if cancel then return
std = cfile(path+std,WIDGET_ID=state.w.fire_xtellcor_ld_base,CANCEL=cancel)
if cancel then return
...
if cancel then return
obj = cfile(path+obj,WIDGET_ID=state.w.fire_xtellcor_ld_base,CANCEL=cancel)
if cancel then return
...
if strmid(obj,strpos(obj,'.',/REVERSE_S)+1) eq 'dat' then begin
   ...
   if n_elements(junk[*,0]) gt 2 then es = reform(junk[2,*])
    ...
    endelse
      ...
      fire_readspec,std,sspec,stdhdr,stdobsmode,start,stop,shifts,stdnorders,$
         stdnaps,stdorders,$
         stdxunits,stdyunits,slith_pix,slith_arc,slitw_pix,slitw_arc, $
         stdairmass,xtitle,ytitle

      fire_readspec,obj,ospec,objhdr,objobsmode,start,stop,shifts,objnorders,$
         objnaps,objorders,$
      ...
      *state.d.stdhdr    = stdhdr
      ...
      state.d.obsmode = strtrim(stdobsmode,2)
      state.d.disp   = float(fxpar(stdhdr,'D*P*'))
...
state.r.continue=1
 ...
 fire_xtellcor_ld_conkernel,state
...
;******************************************************************************
;
pro fire_xtellcor_ld_getshift,state
...
;******************************************************************************
;
pro fire_xtellcor_ld_writefile,state
...
;    mkhdr,hdr,corspec
   ...
   fxaddpar,hdr,'NAPS',1
    ...
    fire_xvspec,strtrim(path+obj,2)+'.fits'
...
state.w.fire_xtellcor_ld_base = widget_base(TITLE='XTELLCOR for FIRE LD Mode', $
                                    EVENT_PRO='fire_xtellcor_ld_event',$
                                    /COLUMN)
    
   quit_button = widget_button(state.w.fire_xtellcor_ld_base,$
                               FONT=buttonfont,$
                               VALUE='Done',$
                               UVALUE='Quit')
       
   state.w.message = widget_text(state.w.fire_xtellcor_ld_base, $
                                 VALUE='',$
                                 YSIZE=1)

   row_base = widget_base(state.w.fire_xtellcor_ld_base,$
                          /ROW)
...
;                                   FONT=buttonfont)
   ...
   help = widget_button(state.w.fire_xtellcor_ld_base,$
                        VALUE='Help',$
                        UVALUE='Help',$
                        FONT=buttonfont)
...
centertlb,state.w.fire_xtellcor_ld_base
widget_control, state.w.fire_xtellcor_ld_base, /REALIZE

; Start the Event Loop. This will be a non-blocking program.

XManager, 'fire_xtellcor_ld', $
  state.w.fire_xtellcor_ld_base, $
  CLEANUP='fire_xtellcor_ld_cleanup',$
  /NO_BLOCK
