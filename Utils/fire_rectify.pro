...
;                      to reddest.
;   
;         outerr     - String for output rectified error file (if desired)
;
...
pro fire_rectify, extraction, orderfile, outfile, $
  ...
  if (keyword_set(INVVAR) OR keyword_set(BPM)) then begin
     if (keyword_set(INVVAR)) then begin
        raw = xmrdfits(extraction, 1, hdr, /silent)
     endif else begin
        raw = xmrdfits(extraction, 6, hdr, /silent)
        err = 1./sqrt(xmrdfits(extraction, 1, /silent))
     endelse
  endif else begin
  ...
  hdr_raw = headfits(extraction)
  exptime = sxpar(hdr_raw, "EXPTIME")
...
;       in pushing the order boundaries.
     ...
     if (keyword_set(BILINEAR)) then begin
        ...
        err_rect = interpolate(err, ximg_rect, yimg_rect-offset)
     endif else begin
        img_rect = interpolate(raw, ximg_rect, yimg_rect-offset, cubic=-0.5)
        err_rect = interpolate(err, ximg_rect, yimg_rect-offset, cubic=-0.5)
     endelse
     ...
     if (iord EQ 0) then begin
        sxdelpar, hdr_raw, "naxis"
        sxdelpar, hdr_raw, "naxis1"
        sxdelpar, hdr_raw, "naxis2"
        mwrfits, img_rect, outfile, hdr_raw, /create 
        if (keyword_set(OUTERR)) then begin
           mwrfits, err_rect, outerr, /create
        endif
        if (keyword_set(WAVEIMG)) then begin
           mwrfits, wv_rect, waveoutpt, hdr_raw, /create
        endif
     endif else begin
        mwrfits, img_rect, outfile
        if (keyword_set(OUTERR)) then begin
           mwrfits, err_rect, outerr
        endif
        if (keyword_set(WAVEIMG)) then begin
