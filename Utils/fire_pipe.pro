...
;  if NOT keyword_set(PLOT_INFO) then NOPLOT = 1
  ...
  for itarg=0, ntargs-1 do begin
     ...
     sci = where(strtrim(fire.object,2) EQ strtrim(targetnames[itarg],2) AND $
     ...
     for iframe=0, nmatch-1 do begin
        ...
        print, '********************************************'
        print, 'Reducing object file: ', scifil1
        print, '********************************************'
		...
		fire_proc, strtrim(skyfil,2), skyimg, skyivar, hdr=scihdr, $
           ...
           if not keyword_set(SEP_SKY_EXP) then begin
              ...
              if (0) then begin
                 ...
                 sky_piximg = fire_makescipix(skyimg, tset_slits, arc2=arcimg)

                 print, "-----------------------------------------------------------------------"
                 print, 'Cross-correlating blank-sky and science frames to measure flexure'
                 print, 'This may take a minute'
                 print, "-----------------------------------------------------------------------"
                 ccorr = convol_fft(skyimg, sciimg, /correlate)
                 ycc = reform(ccorr[1023,1023-5:1023+5])
                 xcc = -5 + findgen(11)
                 xfine = -5. + 0.01 * findgen(1100)
                 coeffs = poly_fit(xcc, ycc, 4)
                 yfine = poly(xfine, coeffs)
                 maxpos = where(yfine EQ max(yfine))
                 shiftpix = xfine[maxpos[0]] 
                 print, "Done...applying best-fit flexure correction of "+strtrim(shiftpix,2)+' pixels.'
                 if (abs(shiftpix) GT 2.0) then begin
                    print, "Warning: Found an anomalously high pixel shift, something is wrong.  Setting flexure to zero instead."
                    print, 'If you really insist, you can set the flexure by hand using shiftpix, see fire_pipe.pro'
                    shiftpix = 0
                    stop
                 endif
                 skyimage = fire_skymodel(sciimg=skyimg, sciivar=skyivar, piximage=sky_piximg, tset_slits=tset_slits, shiftpix=shiftpix)

                 readcol, getenv('FIRE_DIR')+'Arcs/FIRE_OH_R6000.lst', ohwv, ohstrength
                 gdoh = where(ohstrength GT 1.0, noh)
                 ; wavelengths are not yet heliocentric corrected (this happens at extraction)
                 arcimg = 10^xmrdfits(wcimg)
                 ohmask = 0 * arcimg
                 for ioh=0, noh-1 do begin
                    flag = where(abs(arcimg-ohwv[gdoh[ioh]])/ohwv[gdoh[ioh]] LT 50./299793, nflag)
                    ohcorrect = 0
                    if (nflag GT 0 AND ohcorrect) then begin
                       ratio = median(sciimg[flag] / skyimage[flag])
                       skyimage[flag] *= ratio
                       ohmask[flag] = ohstrength[gdoh[ioh]]
                    endif
                 endfor
                 print, ' '
              endelse

;              postfix = strsplit(scifil, '_', /extract)
;              ohmaskfil = 'Arcs/OHmask'+strtrim(postfix[n_elements(postfix)-1],2)
;              mwrfits, ohmask, ohmaskfil, /create

	   endelse
           
           ;;;;;;;;;;;;;;;;;; Object Finding ;;;;;;;;;;;;;;;;;;;;;;;;;

           ; Here, objinter runs user object finding: 1=xatv_xy GUI, 0=none
           ; the chk just tells if you want to look at the results.
           ; If chkobj=3, then it calls for the user interpolation table
           ; If chkobj=4, then it uses the profile from the 
           ;     standard star extractions
           ; This last option is for the case where all else fails. 

           imgminsky=sciimg-skyimage
           objfindimage = imgminsky
           chkobj_local = chkobj

           objinter = (chkobj EQ 2) ? 1 : 0
           if (chkobj EQ 3) then begin
              prof_tabinput = 1
              objinter = 0
           endif
           if (chkobj EQ 4) then begin
              refstarfile = strtrim(scistruct.rawpath,2)+strtrim(scistruct.objtracefile,2)
              if (file_test(refstarfile) NE 1) then begin
                 print, ' '
                 print, 'It appears that you are asking to use a reference star to'
                 print, ' trace the object location, but you have not specified'
                 print, ' the reference star file associated with this science frame'
                 print, ' Stopping for post-mortem.'
                 stop
              endif else begin
                 print, ' '
                 print, ' '
                 print, 'Using trace reference star: '+strtrim(scistruct.objtracefile,2)
                 print, ' will take extra time to sky-subtract before tracing...'
                 print, ' '
                 fire_proc, strtrim(refstarfile,2), refimg, refivar, hdr=refhdr, $
                            pixflatfile=strtrim(pixflat,2),$
                            illumflatfile=strtrim(illum,2), /MASKBADPIX
                 refsky = fire_skymodel(sciimg=refimg, sciivar=refivar, piximage=piximg, $
                                        tset_slits=tset_slits, shiftpix=shiftpix)
                 objfindimage = refimg - refsky
                 chkobj_local = 0
              endelse
           endif

           titlestr = "Object finding for frame: "+strtrim(scifil1,2)
           obj_strct = fire_findobj(img_minsky=objfindimage,$
                                    sciivar=sciivar,waveimg=10^waveimg,$
                                    tset_slits=tset_slits,filstd=filstd, $
                                    nfind=1, chk=chkobj_local, inter=objinter, $
                                    BOX_RAD=box_rad, title=titlestr, $
                                    prof_tabinput=prof_tabinput, $
                                    PROF_IMG=prof_image)
	...
	;; Determine the name of the output structure file
           ...
           if (x_chkfil(objstrfil, /SILENT) EQ 0) OR $
              ...
              result = fire_echextobj(sciimg=sciimg, sciivar=sciivar, $
                                      ...
                                      tset_slits=tset_slits, $
                                      objfile=objstrfil, $
                                      objstr=obj_strct, outfil=outfil, /STD, $
                                      NOPLOT=noplot, $
                                      /HELIO_CORR, REDUXDIR=reduxdir, $
                                      CHK=chkobj_local, OBJNAME=tobjname)
              if result NE 0 then begin
