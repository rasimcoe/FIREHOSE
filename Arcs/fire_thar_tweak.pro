...
;  tharwv = tharwv * gamma
  ...
  for order = 31, 11, -1 do begin
     ...
     ; Fit a 1D spectrum to wavelengths (as determined from ThAr) 
     ; Versus OH sky flux (from the sky frame).  The wavelengths
     ; for this combination should be off by 0.5 pixels or so.
     ...
     ; The archival set is taken from a long exposure
     ; where the OH lines have been calibrated well.
     y_thisframe = bspline_valu(xx, sset_thisframe)
     y_archival  = bspline_valu(xx, sset_archival)

     lag = -50 + findgen(100)           ; Lag in 1/4 pixel increments
     vel_offset = lag * dv              ; Lag in velocity
     firepix_offset = vel_offset / 12.5 ; Lag in fracitonal FIRE pixels
     result = c_correlate(y_thisframe, y_archival, lag) ; Figure out how many pixels we're off by

     ; We expect the cross correlation
     ; should peak near zero lag (middle of 100 element array), 
     ; so enforce this
     fitwidth = 10
     ...
     ; Peak up on the cross correlation maximum.
     ; Use the center value of a gaussian fit 
