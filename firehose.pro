 ...
 'go_trace': BEGIN
    ...
    if bad_input EQ 0 then begin
       ...
       if (x_chkfil(tflt_file[0]) GT 0) then begin
          ...
          tset_slits=fire_findslits(traceflat=tflt_file,  /CLOBBER, /CHK, $
                                   INTER=inter)
                                ;                                 orderfile=orderfile, $
...
;  arcverb_help = widget_button(ab2,value="?", uval='wpix_help')
  ...
  find_options = ["Automated", "Inspect Results", "Interactive/User Fit","Interpolation Table","Reference Star"]
