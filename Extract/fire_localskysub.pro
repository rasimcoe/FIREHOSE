...
;            if (i1 EQ 19) then stop
            ...
            if (keyword_set(PROFILE_INPUT)) then begin
               ...
               ; This vector shows the width of the slit in each order,
               ; in units of pixels.  If SNR < SN_GAUSS, then the profile
               ; is by fiat a gaussian of fwhm = thisfwhm in gprofile
               ; We must enforce that the gaussian tracks anamorphic
               ; distortion of the prisms, otherwise the flux calibration
               ; varies across the spectrum.
               slitwidth_fit = [78.1745,76.7884,75.4161,74.0577,72.7131,71.3824,$
...
;            stop
        ...
        if not keyword_set(NOLOCAL) then begin
            ...
            igood = WHERE(skymask[isub[ithispix]] GT 0 AND chi2 LE 36.0D*5.0d, ngd)
...
; DANGER!!! TAKE THIS OUT LATER.  ONLY MASK ON LATER ITERATIONS
                if (iiter GT 1) then begin
...
;        x_splot, piximg[plt], sciimg[plt], psym1=3, ytwo=skyimage[plt], psym2=3, /block
        ...
        if (0) then begin
           xatv, thismask*outmask*(sciimg-skyimage), /block, min=-100, max=15000
           stop
        endif
