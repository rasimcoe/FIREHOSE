...
FUNCTION fire_findobj, IMG_MINSKY=img_minsky, SCIIVAR=sciivar,$
                       ...
                       FWHM = FWHM, PROF_TABINPUT=prof_tabinput, $
...
;;  Optional Keywords
  ...
  if (keyword_set(INTER) OR keyword_set(PROF_TABINPUT)) then begin
     ...
     if (is_undefined(prof_fitstr) EQ 0) then begin
        ...
        for iord=0, norders-1 do begin
           ...
           ; ATTENTION: THIS IS USED TO SET THE WIDTH OF THE OBJECT MASK
           ; FOR MODELING OF THE SKY
     ...
     chk_local = (chk EQ 4) ? chk : chk ; Turn off object finder checking for reference star mode
  ...
  if (chk EQ 4 AND 0) then begin
     ; Make a profile image
     ximg    = long_slits2x(tset_slits)
     ordermask = fire_ordermask(tset_slits)
