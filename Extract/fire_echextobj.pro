...
;  IF NOT KEYWORD_SET(SN_GAUSS) THEN SN_GAUSS = 100.0d ; FOR TESTING ONLY
...
;;
  ...
  FOR iobj = 0L, nobj-1L DO BEGIN
     FOR iorder = 0L, nord-1L DO BEGIN
        ...
        ; HERE WE ARE FITTING TO FIND THE PROFILE FWHM IN EACH ORDER
        objctr = objstr[sci1].xfracpos
        fwhm_use = (flux_sm * sqrt(fluxivar_sm) GT 2.) ## (fltarr(2048)+1.)
        xx = ximg[where(ordermask EQ 31L-qq)] - objctr
        norm = bspline_valu(wave, b_answer) ## (fltarr(2048)+1.)
        yy = (sciimg[where(ordermask EQ 31L-qq)]-skyimage[where(ordermask EQ 31L-qq)]) $
             / (norm[where(ordermask EQ 31L-qq)])
        ee = 1/sqrt(sciivar[where(ordermask EQ 31L-qq)]) / (norm[where(ordermask EQ 31L-qq)])

        use = where(abs(xx) LT 0.25 AND abs (yy) LT 0.8)

        xbin = -0.25 + 0.5 * findgen(50)/50.
        ybin = fltarr(50)
        for ibin=1, 49 do begin
           inbin = where(abs(xx-xbin[ibin]) LT 0.5/50., nbin)
           if (nbin GT 10) then  begin
              ybin[ibin] = median(yy[where(abs(xx-xbin[ibin]) LT 1./50.)])
           endif
        endfor
        colors=getcolor(/load)
        gfit = gaussfit(xbin, ybin, c, estimates=[0.5,0,0.1], nterms=3)
        ordr_fwhm[iorder,iobj] = c[2]*2.35

        plot, xx[use], yy[use], yrange=[-1,1], psym=3, title='FWHM = '+strtrim(c[2]*2.35*6.,2)+' arcsec'
        oplot, xbin, ybin, color=colors.red, thick=3
        oplot, xbin, ybin, color=colors.green, psym=1, thick=2
        oplot, xbin, gfit, color=colors.blue
  ...
  gdordr = where(ordr_s2n[*,0] GT 3.0, ngdordr)
  if (ngdordr GT 3) then begin
     ave_fwhm = median(ordr_fwhm[gdordr,0])
  endif else begin
     ave_fwhm = 0.7 / 6.0 ; Fraciton of slit, assuming 0.7" seeing
  endelse

  slitwidth_pix = [78.1745,76.7884,75.4161,74.0577,72.7131,71.3824,$
                   70.0656,68.7625,67.4734,66.1981,64.9366,63.6890,62.4552,$
                   61.2353,60.0292,58.8370,57.6586,56.4941,55.3434,$
                   54.2066,53.0836]
  objstr.fwhm = ave_fwhm * slitwidth_pix
